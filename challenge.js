...
// Compute cumulative miles for a single athlete for the selected month
...
// Render the monthly challenge view
export function renderChallengeView(athletesData, monthNames, monthIndex, CHARTS) {
    ...
    leaderboardEl.innerHTML = top3.map(p =>
        `<li><img src="${p.img}" style="width:24px;height:24px;border-radius:50%;margin-right:5px;">${p.name}: ${p.total} mi</li>`
    ).join('');

    // Wait for images to load before rendering chart
    Promise.all(imageLoads).then(() => {
        if (CHARTS.challengeLine) {
            CHARTS.challengeLine.destroy();
            delete CHARTS.challengeLine;
        }
